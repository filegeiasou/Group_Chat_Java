...
public class Client extends JFrame implements ActionListener, MouseListener, KeyListener
{
    ...
    public Client(String username, String password,Socket socket , int a)
    {
        try{
            this.bufferedWriter = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream()));
            this.bufferedReader = new BufferedReader(new InputStreamReader(socket.getInputStream()));
...
class CredentialsHandler extends JFrame implements ActionListener, KeyListener
{
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        {            
            new Client(username.getText(), password.getText(),socket,3);
            if(banana.equals("true"))
            {
                setVisible(false);
            }
            Client c1 = new Client(username.getText(), password.getText(),socket);
            // c1.sendMessage();
            c1.ListenForMessage();
        }
    }
    ...
    @Override
        public void keyPressed(KeyEvent e)
        {
            if(e.getKeyCode() == KeyEvent.VK_ENTER)
            {
                ...
                {
                    new Client(username.getText(), password.getText(),socket , 4);
                    Client c1 = new Client(username.getText(), password.getText(),socket);
                    // c1.sendMessage();
                    setVisible(false);
                    c1.ListenForMessage();
